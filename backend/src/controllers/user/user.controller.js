...
const { securePassword } = require("../../utils/securePassword.util");
const { TYPE_USER, PASSWORD_DEFAULT } = require("../../utils/constants.util");
...
const getUsers = async (req, res) => {
  ...
  try {
    ...
    const users = await User.find()
      ...
      .where(isActive !== undefined ? { isActive } : null)
      .where(userType && userType !== undefined ? { userType } : null)
      .where(userTypes ? { userType: { $in: userTypes } } : null)
      .where(isActives ? { isActive: { $in: isActives } } : null)
